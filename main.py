...
login_manager.login_message = "Увійдіть до сістеми щоб забронювати турік"
...
def signup():
    ...
    if signup_form.validate_on_submit():
        ...
        with Session() as session:
            ...
            if user:
                flash("Уже є користувач з таким логіном")
            ...
            session.add(user)
            session.commit()
            flash("Вітаю, ви ввійшли в систему, можете взламувати людей")
            return redirect(url_for('login'))
    ...
    if login_form.validate_on_submit():
        ...
        with Session() as session:
            ...
            if not user or not check_password_hash(user.password, login_form.password.data):
                flash("Немає такого користувача, не сігма, або паголь не вірний :( ")
                return redirect(url_for('login'))
...
@login_required
def logout():
    flash("До побачення, плакі плакі ")
    logout_user()
...
def reserve(tour_id: int):
    with Session() as session:
        ...
        user = session.query(User).where(User.id == current_user.id).first()
        user.tour.append(tour)
        session.commit()
        flash("Тур заброньований ещкеря")
        return redirect(url_for('cabinet'))
